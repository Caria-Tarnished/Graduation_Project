...
- **情感分析工作流程说明（2026-02-11）**：
  ...
  **工作流程**：
  ...
  用户点击事件详情
      ↓
  Charts 页面调用 Agent.process_query()
      ↓
  Agent 获取市场上下文（get_market_context）
      ↓
  Agent 调用 BERT 模型（SentimentAnalyzer.analyze）
      ↓
  BERT 模型预测基础情感（-1/0/1）
      ↓
  规则引擎后处理（预期兑现、建议观望）
  ...
  **get_market_context 说明**：
  ...
  - 计算指标：
    ...
    - 波动率（volatility）：`(price_high - price_low) / price_start`
    - 趋势标签（trend_tag）：Strong Rally / Sharp Decline / High Volatility / Mild Rally / Weak Decline / Sideways
  - 用途：
    - BERT 模型输入（添加市场上下文前缀）
...
- **Charts 页面修复（2026-02-11）**：
  - **问题 1：页面刷新导致图表消失**
    ...
    - 效果：用户可以自由选择事件查看详情，图表不会消失
  - **问题 2：快讯类事件未显示在 K 线图上**
    - 原因：大部分快讯没有星级（25,057 个快讯中只有 1,109 个有星级）
    - 原因：默认 `min_star=3` 过滤掉了所有没有星级的快讯
    - 解决：修改星级筛选范围为 0-5，默认值改为 0（显示所有事件）
    - 解决：修改 SQL 查询，当 `min_star=0` 时包含 NULL 星级的事件
    - 效果：现在可以看到快讯类事件（需要 BERT 模型分析）
  - **问题 3：affect 标签未使用**
    - 发现：数据库中有 990 个事件有 affect 标签（利多/利空 金银）
    - 解决：SQL 查询包含 affect 字段
    - 解决：情感判断优先使用 affect 标签（准确度 100%）
    - 解决：事件详情显示 affect 标签（带颜色）
  - **情感判断优先级**（新）：
    1. affect 标签（最高优先级，准确度 100%）- 用于日历类事件
    2. 数值比较（中等优先级，准确度 ~70%）- 用于数值型事件
    3. 关键词匹配（最低优先级，准确度 ~50%）- 用于文本型事件
    4. BERT 模型（点击详情时使用）- 用于快讯类事件
  - **数据统计**：
    - 事件总数：26,182
    - 快讯类 (flash)：25,057 (95.7%)，大部分无星级，无 affect 标签
    - 日历类 (calendar)：1,125 (4.3%)，全部有星级，全部有 affect 标签
    - 有 affect 标签：990 (3.8%)
  - **相关文件**：
    - `app/hosts/streamlit_app/pages/2_Charts.py`（主要修改）
    - `app/core/orchestrator/tools.py`（增强调试信息）
