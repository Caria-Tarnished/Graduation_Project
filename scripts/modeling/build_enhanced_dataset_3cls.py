# -*- coding: utf-8 -*-
"""
Data augmentation script for 3-class dataset: Add market context prefixes (Plan A)
...
Input: train/val/test_3cls.csv (with pre_ret, range_ratio features)
Output: train/val/test_enhanced_3cls.csv (with new text_enhanced column)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add market context prefixes to 3-class financial event texts (Plan A)"
    )
    parser.add_argument(
        "--input_dir",
        type=str,
        default="data/processed",
        help="Input directory (containing train/val/test_3cls.csv)"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="data/processed",
        help="Output directory (will generate train/val/test_enhanced_3cls.csv)"
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print("Data Augmentation: Add Market Context Prefixes (3-Class)")
    print("=" * 60)
    ...
    for split in ['train', 'val', 'test']:
        input_file = os.path.join(args.input_dir, f"{split}_3cls.csv")
        output_file = os.path.join(args.output_dir, f"{split}_enhanced_3cls.csv")
    ...
    print("=" * 60)
    print("\nNext steps:")
    print("1. Check generated files: data/processed/*_enhanced_3cls.csv")
    print("2. Verify prefix distribution is reasonable")
    print("3. Train model with enhanced 3-class data:")
    print("   python scripts/modeling/bert_finetune_cls.py \\")
    print("     --train_csv data/processed/train_enhanced_3cls.csv \\")
    print("     --val_csv data/processed/val_enhanced_3cls.csv \\")
    print("     --test_csv data/processed/test_enhanced_3cls.csv \\")
    print("     --output_dir models/bert_3cls_v1 \\")
    print("     --label_col label \\")
    print("     --text_col text_enhanced \\")
